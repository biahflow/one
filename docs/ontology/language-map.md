...
**Feito até 02/09/2026:** `Client`→`Account` e o Engagement como nível entre conta e projeto (ADR 0079); o guard de visibilidade da §3 (ADR 0082); o portão de vocabulário sobre os dois deployables (ADR 0083) e o rótulo de projeção do ROI (ADR 0084); **KPI · Measurement · Baseline · Outcome · Value Ledger**, que passaram a atravessar o snapshot e a ser exibidos ao cliente (ADR 0085) — com os invariantes 11 e 12 da §6 verificados por teste pela primeira vez; e **Process · ProcessStep · Finding · Evidence · PainPoint · ImprovementOpportunity · PriorityAssessment · SolutionHypothesis**, na aba Discovery, com o invariante 9 verificado por teste (ADR 0086). **Nada falta do lado do One.**

**A pendência de documento da §3 foi resolvida por medição, e não por código** (ADR 0086). A linha "Evidence marcada como revisada e publicável" está certa e continua valendo — só a Evidence revisada atravessa —, mas **a marca não é um campo do payload**: `published_at`/`published_by` existem só no modelo do Pulse, e o filtro é aplicado *antes* de emitir, de modo que a presença da evidência no array já é a prova. O One não declara campo de revisão, e não declará-lo é a decisão: um campo que não vem de lugar nenhum seria o One afirmando por conta própria que algo foi revisado, contra a regra 3 desta seção. O portão que vigia isso é o `reviewed_resources` de `docs/contracts/one-visibility.json`, que reprova no dia em que a origem passar a emitir a marca.
