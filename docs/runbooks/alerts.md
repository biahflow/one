...
| `projection.phase_ref_unresolved` | qualquer ocorrência repetida no mesmo `project_id`; 5 em 1 h | Uma decisão veio com `phase_ref` **preenchido** e ele não casou com nenhuma fase do **mesmo envelope** (ADR 0088). A decisão foi gravada sem âncora, então nada se perdeu na aba Decisões — o que se perde é o nó dela na timeline, e o cliente deixa de ver qual decisão destravou aquela fase. **Não confunda com o legado:** `phase_ref` nulo é a origem dizendo que não ancorou, não emite nada e é o caso normal; uma linha aqui quer dizer que a origem **carimbou algo que não chegou**, e as duas causas são do produtor — a fase saiu do `journey.phases[]` do snapshot (arquivada, removida, ou o envelope veio truncado) enquanto a decisão continuou apontando para ela, ou os dois blocos foram montados a partir de leituras diferentes do banco de lá. Repetido no mesmo projeto é o que importa: isolado é a corrida de uma fase removida entre duas leituras; repetido é inconsistência estável, e aí a conferência é no Pulse (ADR 0057 de lá), não aqui. **Não há conserto deste lado, e é de propósito:** inferir a fase por `decided_on` foi recusada em dois gates humanos. `extra` traz `project_id`, `biahflow_project_id`, `phase_ref` (o id não resolvido) e `decision_external_id`; **nada** do título nem do racional da decisão. | `integracao-biahflow.md` |
