...
locals {
  ...
  # Imagem declarada no serviço vence: é o caso do Keycloak, que não construímos.
  # Para as nossas vale a regra de sempre — bootstrap no primeiro apply, `<registro>/
  # <nome>:<sha>` depois, com o `ignore_changes` do módulo protegendo o que o deploy
  # publicou.
...
locals {
  ...
  segredos_do_produto = toset([
    for nome, dono in local.fundacao.segredos : nome if dono == "portal"
  ])
...
resource "terraform_data" "portoes_de_segredo" {
  ...
  lifecycle {
    ...
    precondition {
      ...
      error_message = format(
        "Segredo do portal do cliente que nenhum serviço deste state lê: %s. Alguém põe valor nele, o `gcloud secrets versions add` responde sucesso, e o controle que ele sustenta continua desligado.",
