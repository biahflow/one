...
The repo is a monorepo at the "foundation" stage, and the two halves are at different stages. **The API is real**: OIDC token validation, membership-based authorization and Row-Level Security are implemented (ADR 0010) — every client endpoint requires `Authorization: Bearer` and answers 404, never 403, on a denial. **The web layer is not yet**: it has no session, so the API answers 401 and `app/page.tsx` falls back to `DEMO_OVERVIEW`. Auth.js in the BFF, the `/login` screen and the end of that fallback are the next slice; RAG and the admin UI are further out (`ROADMAP.md`). When changing behavior, check which side you are on.
