...
variable "acesso" {
  description = <<-TXT
    ...
    cliente não sabe assinar**. A `pulse-api` é chamada pelo nginx que a
    `pulse-web` executa e pelo relay do site de marketing; nginx não emite ID
...
resource "google_cloud_run_v2_service" "servico" {
  ...
  # default do Cloud Run e para a `pulse-api` estaria errado: quem a chama é o
...
#
#   `publico`         → qualquer um na internet invoca. É o caso da `pulse-web`,
