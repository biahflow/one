...
- Agent keys and financial assumptions live in `admin.py` too, for the same reason: they are the write path under `portal_admin`. A plaintext key is returned exactly once, at creation; nothing anywhere can recover it afterwards.
